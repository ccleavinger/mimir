...
use super::{
    ir::{MimirBinOp, MimirExprIR, MimirLit, MimirPtrType, MimirType, MimirVariable},
    util::expr_to_ty,
};
...
impl SpirVCompiler {
    ...
    pub fn stmt_to_ir(&mut self, stmt: &syn::Stmt) -> Result<(), String> {
        match stmt {
            syn::Stmt::Local(local) => {
                ...
                self.ir.push(MimirExprIR::Local(
            ...
            syn::Stmt::Item(_) => Err("Item declarations not supported".to_string()),
    ...
    pub fn expr_to_ir(&mut self, expr: &syn::Expr) -> Result<Option<MimirExprIR>, String> {
        match expr {
            ...
            syn::Expr::Async(_expr_async) => Err("Async expressions not supported".to_string()),
            syn::Expr::Await(_expr_await) => Err("Await expressions not supported".to_string()),
            syn::Expr::Binary(expr_binary) => Ok(Some(self.binary_expr_to_ir(expr_binary, false)?)),
            syn::Expr::Block(_) => Err("Block expressions not supported currently".to_string()),
            syn::Expr::Break(_) => Err("Break expressions not supported currently".to_string()),
            syn::Expr::Call(_) => Err("Function calls not supported currently".to_string()),
            syn::Expr::Cast(_) => Err("Casting not supported currently".to_string()),
            syn::Expr::Closure(_) => Err("Closures aren't permitted.".to_string()),
            syn::Expr::Const(_) => Err("Const expressions not supported currently".to_string()),
            syn::Expr::Continue(_) => {
            ...
            syn::Expr::ForLoop(expr_for_loop) => {
                ...
                let (var1, var2) = match *expr_for_loop.expr.clone() {
                    syn::Expr::Range(expr_range) => {
                        let start = match &*expr_range
                            .start
                            .ok_or_else(|| "Expected start of range".to_string())?
                        {
                        ...
                        let end = match &*expr_range
                            .end
                            .ok_or_else(|| "Expected end of range".to_string())?
                        {
                            syn::Expr::Lit(expr_lit) => expr_lit
                                .lit
                                .to_token_stream()
                ...
                let ir: Vec<_> = expr_for_loop
                    ...
                    .stmts
                    .iter()
                    .map(|stmt| self.stmt_to_var_ir(stmt))
                    .collect::<Result<_, _>>()?;

                Ok(Some(MimirExprIR::For(var_name, var1, var2, ir)))
            }
            syn::Expr::Group(_) => Err("Group expressions not supported currently".to_string()),
            syn::Expr::If(expr_if) => {
                let condition = self
                    .expr_to_ir(&expr_if.cond)?
                    .ok_or_else(|| "Expected expression for condition".to_string())?;

                let then = self
                    .expr_to_ir(&syn::Expr::Block(syn::ExprBlock {
                        attrs: Vec::new(),
                        label: None,
                ...
                Ok(Some(MimirExprIR::If(
                    Box::new(condition),
                    Box::new(then),
                    else_.and_then(|ir| ir.map(Box::new)),
                )))
            }
            syn::Expr::Index(expr_index) => {
                ...
                let index = self
                    ...
                    .ok_or_else(|| "Expected expression for index".to_string())?;

                Ok(Some(MimirExprIR::Index(var_name, Box::new(index))))
            }
            syn::Expr::Infer(_) => Err("Infer expressions not supported currently".to_string()),
            syn::Expr::Let(_) => Err("Let gaurd expressions not supported currently".to_string()),
            syn::Expr::Lit(expr_lit) => {
                let lit = match &expr_lit.lit {
                    syn::Lit::Int(lit_int) => {
                        MimirLit::Int32(lit_int.base10_parse::<i32>().unwrap())
                    }
                    syn::Lit::Float(lit_float) => {
                        MimirLit::Float32(OrderedFloat(lit_float.base10_parse::<f32>().unwrap()))
                    }
            ...
            syn::Expr::Range(_) => Err("Range expressions not supported currently".to_string()),
            syn::Expr::RawAddr(_) => {
            ...
            syn::Expr::Repeat(_) => Err("Repeat expressions not supported currently".to_string()),
            syn::Expr::Return(_) => Err("Return expressions not supported currently".to_string()),
            syn::Expr::Struct(_) => Err("Struct expressions not supported currently".to_string()),
            syn::Expr::Try(_) => Err("Try expressions not supported currently".to_string()),
            syn::Expr::TryBlock(_) => {
                Err("Try block expressions not supported currently".to_string())
            }
            syn::Expr::Tuple(_) => Err("Tuple expressions not supported currently".to_string()),
            syn::Expr::Unary(_) => Err("Unary expressions not supported currently".to_string()),
            syn::Expr::Unsafe(_) => Err("Unsafe expressions not supported currently".to_string()),
            syn::Expr::Verbatim(_) => {
                Err("Verbatim expressions not supported currently".to_string())
            }
            syn::Expr::While(_) => Err("While expressions not supported currently".to_string()),
            syn::Expr::Yield(_) => Err("Yield expressions not supported currently".to_string()),
            _ => todo!(),
        }
    }
    ...
    pub fn binary_expr_to_ir(
        ...
        expr_bin: &ExprBinary,
        paren: bool,
    ) -> Result<MimirExprIR, String> {
        let lhs = self
            .expr_to_ir(&expr_bin.left)?
            .ok_or_else(|| "Expected expression from left hand side".to_string())?;
        let rhs = self
            .expr_to_ir(&expr_bin.right)?
            .ok_or_else(|| "Expected expression from right hand side".to_string())?;
